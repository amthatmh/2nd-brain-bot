...
def _store_extracted_strength_state(cf_pending: dict, key: str, extracted: dict | None, raw_text: str | None = None) -> dict:
    ...
    state["sets_breakdown"] = extracted.get("sets_breakdown") or None
...
async def _send_notes_prompt(message, key: str, cf_pending: dict, prefix: str | None = None, body: str | None = None) -> None:
    ...
    sent = await message.reply_text(prompt, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Skip", callback_data=f"cf:skip:{key}")], [InlineKeyboardButton("❌ Cancel session", callback_data="cf:cancel")]]))
    state = cf_pending.get(key, {})
    state["_strength_notes_prompt_id"] = getattr(sent, "message_id", None)
    state["__strength_notes_prompt_id_message"] = sent
    cf_pending[key] = state
...
async def _prompt_track_or_notes(message, key: str, notion, config: dict, cf_pending: dict) -> bool:
    """Select today's Workout Days relation before final prompts."""
    state = cf_pending.get(key, {})
...
async def _continue_strength_after_notes(message, key: str, notion, config: dict, cf_pending: dict, notes) -> None:
    state = cf_pending.get(key, {})
    state["notes"] = notes
    cf_pending[key] = state
    ...
    if (
        movement_id
        and len(state.get("movements") or []) == 1
        and await handle_gymnastics_level_check(message, movement_id, movement_name, notion, config, cf_pending, key)
    ):
        return
    await _finalize_flow(message, key, notion, config, cf_pending, notes)
...
async def _prompt_wod_notes(message, key: str, state: dict) -> None:
    state["stage"] = "wod_notes"
    movement_names = ", ".join(state.get("movement_names") or state.get("movements") or [])
    body = "📝 Any notes about this session?"
    if movement_names:
    ...
    prompt_message = await message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("Skip", callback_data=f"cf:skip:{key}")],
            [InlineKeyboardButton("❌ Cancel session", callback_data="cf:cancel")],
        ]),
    )
    state["notes_prompt_message_id"] = getattr(prompt_message, "message_id", None)
    state["_notes_prompt_message_id_message"] = prompt_message
...
async def _fetch_relation_details(
    notion, relation_items: list[dict], request_cache: dict[str, dict]
) -> list[dict]:
    """Return [{"name": str, "scaling_notes": str}, ...] for each related page."""
    details: list[dict] = []
    for rel in relation_items or []:
        page_id = rel.get("id") or rel.get("page_id")
        if not page_id:
        ...
        try:
            ...
            details.append({
                "name": _plain_title(p) or "Movement",
                "scaling_notes": _plain_rich_text_prop(p, "Scaling Notes") or "",
            })
        except Exception as exc:
            log.warning("Could not retrieve relation %s: %s", page_id, exc)
    return details


async def _movement_sub_details(
    notion, movement_page_id: str,
    request_cache: dict[str, dict] | None = None,
    *,
    ...
    if fetch_relations:
        comp = await _fetch_relation_details(
            notion, _relation_items(props, "Complementary Movements"), request_cache
        )
        ant = await _fetch_relation_details(
            notion, _relation_items(props, "Antagonist Movements"), request_cache
        )
    else:
        comp = await _relation_names_cached(
            notion, _relation_items(props, "Complementary Movements"), request_cache
        )
        ant = await _relation_names_cached(
            notion, _relation_items(props, "Antagonist Movements"), request_cache
        )
    return {
        ...
        "complementary_movements": comp,
        "antagonist_movements": ant,
    }


def _format_sub_search_result(details: dict) -> str:
    def _fmt_relation_list(items) -> str:
        if not items:
            return "  • None set"
        lines = []
        for item in items:
            if isinstance(item, dict):
                name = item.get("name") or "Movement"
                scaling = item.get("scaling_notes") or ""
                lines.append(f"  • *{name}*" + (f": {scaling}" if scaling else ""))
            else:
                lines.append(f"  • {item}")
        return "\n".join(lines)
    ...
    comp_block = _fmt_relation_list(details.get("complementary_movements") or [])
    ant_block = _fmt_relation_list(details.get("antagonist_movements") or [])
    parts = [
...
async def handle_cf_sub_search_reply(message, movement_text: str, notion, config, cf_pending, key: str) -> None:
    ...
    details = await _movement_sub_details(notion, movement_id, {}, fetch_relations=True)
...
def _parse_pr_request(raw_text: str) -> tuple[str, int | None, str | None]:
    """Returns (movement_text, target_reps, since_date_iso)."""
    text = (raw_text or "").strip()
    since_date: str | None = None
    since_re = re.compile(
        r"\bsince=(\d{4}-\d{2}-\d{2}|\d+[dwm])\b", re.IGNORECASE
    )
    since_match = since_re.search(text)
    if since_match:
        text = since_re.sub("", text).strip()
        raw_since = since_match.group(1)
        if re.match(r"\d{4}-\d{2}-\d{2}", raw_since):
            since_date = raw_since
        else:
            qty = int(re.match(r"(\d+)", raw_since).group(1))
            unit = raw_since[-1].lower()
            days = qty if unit == "d" else qty * 7 if unit == "w" else qty * 30
            since_date = (local_today() - timedelta(days=days)).isoformat()
    target_match = re.match(r"(\d+)\s*[xX×]\s*(.+)", text)
    if not target_match:
        target_match = re.match(r"(\d+)\s*(?:reps?|rep)\s+(.+)", text, re.IGNORECASE)
    if target_match:
        return target_match.group(2).strip(), int(target_match.group(1)), since_date
    return text, None, since_date
...
async def _query_workout_log_for_movement(notion, workout_log_db_id: str, movement_id: str, page_size: int = 10, sort_property: str = "load_lbs", sort_direction: str = "descending", after_date: str | None = None) -> list[dict]:
    movement_filter: dict = {"property": "Movement", "relation": {"contains": movement_id}}
    if after_date:
        db_filter: dict = {"and": [
            movement_filter,
            {"property": "Date", "date": {"after": after_date}},
        ]}
    else:
        db_filter = movement_filter
    results = await _maybe_await(notion_call(
        notion.databases.query,
...
async def handle_cf_prs_reply(message, movement_text: str, notion, config, cf_pending) -> None:
    key = str(message.chat_id)
    requested_movement, target_reps, since_date = _parse_pr_request(movement_text)
    ...
    if target_reps is None:
        since_note = f"  _(since {since_date})_" if since_date else ""
        lines = [
            f"🏆 *{movement_name} — PR Summary*{since_note}",
            "",
        ...
        lines = [
            f"🏋️ *{movement_name} — {target_reps} Rep Target*{since_note}",
            "",
...
async def _finalize_flow(message, key, notion, config, cf_pending, notes=None):
    ...
    if state.get("mode") == "strength":
        ...
        sets_breakdown = state.get("sets_breakdown")
        created: list[str] = []
        ...
        if sets_breakdown and len(sets_breakdown) > 1:
            ...
            for idx, movement in enumerate(movements):
                ...
                log.info(
                    ...
                    movement_name, effort_sets, effort_reps, load_lbs, workout_date,
                )
        ...
        result_rounds = None
        result_reps = None
        result_text = state.get("result_notes") or ""
        scaling_notes = notes if notes is not None else state.get("wod_notes")
        if result_text:
            result_seconds = parse_time_to_seconds(result_text)
            rounds, reps = parse_rounds_reps(result_text)
            if rounds is not None:
                result_rounds = rounds
            if reps is not None:
                result_reps = reps
            if result_rounds is None:
                result_rounds = parse_rounds_only(result_text) or parse_emom_rounds(result_text)
            if result_reps is None:
                result_reps = parse_reps_only(result_text)
            if result_seconds is not None:
        ...
        if result_text and result_text.strip() and result_text.strip() not in raw_log:
            raw_log = f"{raw_log}\nResult: {result_text.strip()}" if raw_log else result_text.strip()
        if scaling_notes and scaling_notes.strip() and scaling_notes.strip() not in raw_log:
            raw_log = f"{raw_log}\nNotes: {scaling_notes.strip()}" if raw_log else scaling_notes.strip()
...
async def handle_cf_text_reply(message, text, cf_flow_key, claude, notion, config, cf_pending):
    ...
    if state.get("mode") == "wod" and state.get("stage") == "result":
        ...
        cf_pending[cf_flow_key] = state
        await _prompt_wod_notes(message, cf_flow_key, state)
        return
    if state.get("mode") == "wod" and state.get("stage") == "wod_notes":
        state["wod_notes"] = text
        await _edit_stored_prompt_message(
            message,
            state,
            "notes_prompt_message_id",
            f"📝 Any notes about this session? {_markdown_bold_value(text.strip())}",
        )
    ...
    if state.get("stage") == "notes":
        if state.get("mode") == "strength":
            await _edit_stored_prompt_message(
                message, state, "_strength_notes_prompt_id",
                f"📝 Notes: {_markdown_bold_value(text.strip())}",
            )
            await _continue_strength_after_notes(message, cf_flow_key, notion, config, cf_pending, text)
            return
...
async def _cf_chain_yes(q, parts, claude, notion, config, cf_pending) -> None:
    ...
    label = "B — Strength" if step == "b" else "C — WOD" if step == "c" else step.upper()
    try:
        await q.edit_message_text(f"✅ Section {label}", reply_markup=None)
    except Exception:
        pass
    if step == "b":
        await handle_cf_strength_flow(q.message, {}, claude, notion, config, cf_pending)
...
async def _cf_chain_no(q, parts, claude, notion, config, cf_pending) -> None:
    ...
    try:
        await q.edit_message_text("✅ Session complete.", reply_markup=None)
    except Exception:
...
async def _cf_rx(q, parts, claude, notion, config, cf_pending) -> None:
    ...
    state["_rx_scaled_msg_id"] = getattr(q.message, "message_id", None)
    cf_pending[key] = state
    await q.edit_message_text(f"✅ {_rx_scaled_label(parts[3])}", parse_mode="Markdown")
    if state.get("result_notes") is not None:
        await _finalize_flow(q.message, key, notion, config, cf_pending, state.get("wod_notes"))
    else:
        state["stage"] = "result"
        cf_pending[key] = state
...
async def _cf_skip(q, parts, claude, notion, config, cf_pending) -> None:
    ...
    elif state.get("mode") == "wod" and state.get("stage") == "wod_notes":
        await q.edit_message_text("📝 Notes Skipped", parse_mode="Markdown", reply_markup=None)
        state["wod_notes"] = None
        state["stage"] = "rx_scaled"
        cf_pending[key] = state
        await q.message.reply_text("Rx or Scaled?", parse_mode="Markdown", reply_markup=rx_scaled_keyboard(key))
    elif state.get("stage") == "notes" and state.get("mode") == "strength":
        await q.edit_message_text("📝 Notes Skipped", parse_mode="Markdown", reply_markup=None)
        await _continue_strength_after_notes(q.message, key, notion, config, cf_pending, None)
    else:
        await _finalize_flow(q.message, key, notion, config, cf_pending, None)
