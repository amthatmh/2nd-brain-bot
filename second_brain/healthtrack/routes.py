...
_habits_health_metrics_db: str = ""  # set by register_health_routes
...
def _build_habits_data_payload(
    *,
    ...
    # Sleep minutes from health metrics DB (Total Sleep (min) per date)
    ...
    if health_metrics_db and sleep_habit_name:
        try:
            ...
            for row in metrics_rows:
                ...
                row_date = extract_date_fn(rd.get("start") if rd else None)
                raw_sleep = rp.get("Total Sleep (min)", {}).get("number")
                if row_date and isinstance(raw_sleep, (int, float)) and raw_sleep > 0:
                    sleep_minutes_by_date[row_date] = int(raw_sleep)
