...
CallbackHandler = Callable[
    [object, list[str], ContextTypes.DEFAULT_TYPE], Awaitable[None]
]


def _main():
    import second_brain.main as _m
...
def _rule_engine():
    return _main().rule_engine


def _habit_cache() -> dict:
...
def _trip_map() -> dict:
    return _main().trip_map


def _preview_map() -> dict:
    return _main().preview_map


def _pending_message_map() -> dict:
    return _main().pending_message_map


def _crossfit_config():
...
async def route_classified_message_v10(message, text: str) -> None:
    ...
    if intent == "watchlist":
        ...
        await wl.handle_watchlist_intent(
            ...
            media_type=result.get("media_type", "Series"),
        )
        return
    ...
    if intent == "habit":
        ...
        if habit_name and habit_name in _habit_cache() and confidence == "high":
            ...
            await thinking.edit_text(
                ...
                reply_markup=kb.habit_buttons(all_habits, "manual", selected=set()),
            )
            _main()._store_habit_selection_session(thinking.message_id, all_habits)
        return
    ...
    if intent == "entertainment_log":
        ...
        if date_result and getattr(date_result, "ambiguous", False):
            ...
            await thinking.edit_text(
                ...
                reply_markup=kb.date_pick_keyboard("ent", key, date_result),
            )
            return
        if confidence == "high" and title:
            try:
                ...
                await message.reply_text(
                    "⚠️ I understood that as entertainment, but couldn't save to Notion."
                )
            return
...
async def handle_message_text(
    ...
    if lower.startswith("/signoff"):
        ...
        await _main().trigger_signoff_now(message, note=note or None)
        return
    if lower == "signoff":
        await _main().trigger_signoff_now(message)
        return
    ...
    if not (
        ...
        _main().track_claude_activity(text)

    if lower == "cancel":
        if (
            message.reply_to_message
            and message.reply_to_message.message_id in STATE.digest_map
        ):
            STATE.digest_map.pop(message.reply_to_message.message_id, None)
            await message.reply_text("✅ Dismissed")
            return
        await message.reply_text(
            "Reply to a digest message with `cancel` to dismiss it."
        )
        return
    command_head = lower.split(maxsplit=1)[0] if lower else ""
    command_arg_text = (
        text[len(text.split(maxsplit=1)[0]) :].strip() if text.split(maxsplit=1) else ""
    )

    if (
        message.reply_to_message
        and message.reply_to_message.message_id in _main().trip_awaiting_date_map
    ):
        key = _main().trip_awaiting_date_map.pop(message.reply_to_message.message_id)
        parsed = trips_mod.parse_trip_message(text, _claude())
        dep, ret = parsed.get("departure_date"), parsed.get("return_date")
        if not dep or not ret:
            await message.reply_text(
                "⚠️ I couldn't parse those dates. Try format like Jun 14-17."
            )
            return
        _trip_map()[key]["departure_date"] = dep
        _trip_map()[key]["return_date"] = ret
    ...
    if _main().awaiting_packing_feedback and not command_head.startswith("/"):
        _main().awaiting_packing_feedback = False
        try:
            _notion().pages.create(
                parent={"database_id": NOTION_PACKING_ITEMS_DB},
                properties={
                    "Item": title_prop(text[:100]),
                    "Always": {"checkbox": True},
                },
            )
    ...
    if context.user_data.get("awaiting_mute_days"):
        try:
            ...
            await message.reply_text(
                "Please send a valid positive number of days (example: 3)."
            )
        return
    ...
    if context.user_data.get("awaiting_location"):
        if _main().wx.set_location_smart(text, _claude()):
            ...
            try:
                ...
                await message.reply_text(
                    "⚠️ Weather is temporarily unavailable. Try /weather again in a moment or /location to reset."
                )
        else:
            await message.reply_text(
    ...
    if lower.startswith("weather:"):
        ...
        if lower.startswith("weather:"):
            ...
            if requested_location:
                if not _main().wx.set_location_smart(requested_location, _claude()):
                    await message.reply_text(
                        "Couldn't find that location. Try city/state/country or ZIP (example: Chicago IL 60605)."
                    )
                    return
    ...
    pending_sport_competition = ent_log.pending_sport_competition_map.get(
        ...
        if competition:
            ...
            if page_id and props:
                try:
                    ...
                    ent_log.pending_sport_competition_map.pop(
                    ...
                    await message.reply_text(
                        "⚠️ I couldn't update that competition in Notion."
                    )
                    return
    ...
    if lower in upload_programme_aliases or _main().looks_like_crossfit_programme(text):
        await message.reply_text(
            "📋 Weekly programmes are parsed from Notion only now.\n\n"
    ...
    if pending_custom_topic:
        ...
        entry = _main().pending_note_map.pop(key, None)
        context.user_data["awaiting_note_custom_topic"] = None
        if not entry:
            await message.reply_text(
                "⚠️ This note prompt expired — please re-send the note."
            )
            return
        custom_topic = text.strip()[:60]
        if not custom_topic:
            await message.reply_text(
                "⚠️ Topic can't be empty — please re-send the note."
            )
            return
        try:
            notion_notes.create_note_entry(
                _notion(), NOTION_NOTES_DB, entry["content"], custom_topic
            )
    ...
    if awaiting_note_capture:
        if not NOTION_NOTES_DB:
            ...
            await message.reply_text(
                "📝 Notes DB isn't configured yet — add NOTION_NOTES_DB first."
            )
            return
        if awaiting_note_capture == "link" and not re.search(r"https?://\S+", text):
            ...
            return
        try:
            notion_notes.create_note_entry(_notion(), NOTION_NOTES_DB, text)
            kind_label_map = {
                "quick": "note",
                "idea": "idea",
                "code": "code snippet",
            ...
            await _main().reply_notion_error(message, "save note")
        finally:
            context.user_data["awaiting_note_capture"] = None
        return
    ...
    if match_note:
        STATE.notes_pending.discard(update.effective_chat.id)
        await _main().handle_note_input(message, match_note.group(1).strip())
        return

        # User is in note-capture mode — next message is the note content
    if update.effective_chat.id in STATE.notes_pending:
        await _main().handle_note_input(message, text)
        return

    if lower == "done" and message.reply_to_message:
        replied_id = message.reply_to_message.message_id
        if replied_id in STATE.capture_map:
            captured = STATE.capture_map[replied_id]
            await _main().complete_task_by_page_id(
                message, captured["page_id"], captured["name"]
            )
            return
        if replied_id in STATE.digest_map:
            await message.reply_text(
                "Reply with `done 1` or `done 1,3`, or use `done: task name`.",
                parse_mode="Markdown",
            )
            return

    command_handler = _main().COMMAND_DISPATCH.get(
        lower
    ) or _main().COMMAND_DISPATCH.get(lower_normalized)
    if command_handler:
    ...
    if explicit_entertainment:
        ...
        try:
            ...
            await message.reply_text(
                _main()._entertainment_save_error_text(e, explicit_entertainment)
            )
        return
    ...
    if numbers:
        ...
        if source_id and source_id in STATE.digest_map:
            items = STATE.digest_map[source_id]
            for n in numbers:
                if 1 <= n <= len(items):
            ...
            recovered = notion_tasks.recover_digest_items_from_text(
                _notion(), NOTION_DB_ID, replied_text
            )
            for n in numbers:
        ...
        if done_names:
            ...
            await message.reply_text(
                "No recent digest found. Try replying directly to a digest message."
            )
        return
    ...
    if match_name:
        matched = notion_tasks.notion_tasks.fuzzy_match(
            match_name.group(1).strip(),
            notion_tasks.get_all_active_tasks(_notion(), NOTION_DB_ID),
        )
        if matched:
            await _main().complete_task_by_page_id(
                message, matched["page_id"], matched["name"]
            )
        else:
            await message.reply_text(
    ...
    if match_mark_done:
        ...
        if matched:
            ...
            await message.reply_text(
                f'Couldn\'t find a task matching "{match_mark_done .group (1 ).strip ()}".'
            )
        return
    ...
    if cf_flow_key and cf_flow_key in _cf_pending():
        await _main().handle_cf_text_reply(
            message,
            text,
            cf_flow_key,
            _claude(),
    ...
    if match_force:
        await _main().create_or_prompt_task(
            message, match_force.group(1).strip(), force_create=True
        )
        return
...
async def _cb_confirm_batch(q, parts, context) -> None:
    await _main().on_confirm_batch(SimpleNamespace(callback_query=q), context)
    return


async def _cb_cancel_batch(q, parts, context) -> None:
    await _main().on_cancel_batch(SimpleNamespace(callback_query=q), context)
    return


async def _cb_task_preview(q, parts, context) -> None:
    try:
        preview_message_id = int(parts[1])
    except ValueError:
        await q.edit_message_text(
            "⚠️ This task preview is invalid — please send it again."
        )
        return
    entry = _preview_map().pop(preview_message_id, None)
    if not entry:
        await q.edit_message_text(
            "⚠️ This task preview expired — please send it again."
        )
        return
    action = parts[0]
...
async def _cb_date_pick(q, parts, context) -> None:
    ...
    payload["date"] = (
        payload.get("raw_date_a") if choice == "a" else payload.get("raw_date_b")
    )
...
async def _cb_tcancel(q, parts, context) -> None:
    key = parts[1]
    _trip_map().pop(key, None)
    await q.edit_message_reply_markup(reply_markup=None)
    return


async def _cb_tw(q, parts, context) -> None:
    _, key, slug = parts
    await q.answer()
    if key not in _trip_map():
    ...
    current = _trip_map()[key].get("field_work_types", [])
    if label == "None":
        _trip_map()[key]["field_work_types"] = ["None"]
    elif label in current:
        current.remove(label)
        _trip_map()[key]["field_work_types"] = current
    elif label:
        current = [x for x in current if x != "None"]
        current.append(label)
        _trip_map()[key]["field_work_types"] = current
    new_markup = kb.field_work_keyboard(key, _trip_map())
    current_markup = q.message.reply_markup
    if str(new_markup.inline_keyboard) != str(
        current_markup.inline_keyboard if current_markup else None
    ):
        await q.edit_message_reply_markup(reply_markup=new_markup)
    return


async def _cb_twd(q, parts, context) -> None:
    key = parts[1]
    if key not in _trip_map():
        await q.edit_message_text("⚠️ Trip session expired. Use /trip again.")
        return
...
async def _cb_tms(q, parts, context) -> None:
    _, key, ans = parts
    if key not in _trip_map():
        await q.edit_message_text("⚠️ Trip session expired. Use /trip again.")
        return
...
async def _cb_tcl(q, parts, context) -> None:
    _, key, ans = parts
    if key not in _trip_map():
        await q.edit_message_text("⚠️ Trip session expired. Use /trip again.")
        return
    ...
    await trips_mod.execute_trip(
        ...
        claude=_claude(),
        trip_map=_trip_map(),
        set_awaiting_packing_feedback=lambda value: setattr(
            _main(), "awaiting_packing_feedback", value
        ),
        fetch_weather=_main().wx.fetch_weather,
        fetch_trip_weather_range=_main().wx.fetch_trip_weather_range,
        schedule_weather_refresh=_main().schedule_weather_refresh,
    )
    return


async def _cb_cf(q, parts, context) -> None:
    if len(parts) > 1 and parts[1] == "upload_programme":
        context.user_data["awaiting_programme_upload"] = True
        _cf_pending()["__awaiting_upload__"] = True
        await q.message.reply_text(
    ...
    await _main().handle_cf_callback(
        q,
        parts,
        _claude(),
        _notion(),
        ...
        _cf_pending(),
        chain_after=(
            parts[:2] == ["cf", "log_readiness"] and not q.data.startswith("cf:A")
        ),
    )
    return


async def _cb_kind_task(q, parts, context) -> None:
    key = parts[1]
    text = _pending_message_map().pop(key, None)
    if not text:
        await q.edit_message_text("⚠️ This prompt expired — please send it again.")
        return
    await q.edit_message_text("📌 Routed to task flow.")
    if _main().looks_like_task_batch(text):
        await _main().create_or_prompt_task(q.message, text)
    else:
        await route_classified_message_v10(q.message, text)
    return


async def _cb_kind_refresh(q, parts, context) -> None:
    key = parts[1]
    _pending_message_map().pop(key, None)
    await q.edit_message_text("🔄 Refreshed.")
    await _main().send_quick_reminder(q.message, mode="priority")
    return


async def _cb_mq(q, parts, context) -> None:
    action = parts[1]
    if action == "cancel":
...
async def _cb_nq(q, parts, context) -> None:
    mode = parts[1]
    if mode == "cancel":
...
async def _cb_kind_note(q, parts, context) -> None:
    key = parts[1]
    text = _pending_message_map().pop(key, None)
    if not text:
        await q.edit_message_text("⚠️ This prompt expired — please send it again.")
    ...
    await _main().start_note_capture_flow(q.message, text)
    return


async def _cb_note_topic(q, parts, context) -> None:
    key = parts[1]
    topic_ref = parts[2]
    entry = _main().pending_note_map.get(key)
    if not entry:
        await q.edit_message_text(
            "⚠️ This note prompt expired — please re-send the note."
        )
    ...
    if topic_ref == "add":
        ...
        return
    _main().pending_note_map.pop(key, None)
    if topic_ref == "none":
        selected_topic = None
    else:
        try:
    ...
    try:
        notion_notes.create_note_entry(
            _notion(), NOTION_NOTES_DB, entry["content"], selected_topic
        )
        if selected_topic:
...
async def _cb_notes_start(q, parts, context) -> None:
    STATE.notes_pending.add(q.message.chat_id)
    await q.edit_message_text(
...
async def _cb_h_toggle(q, parts, context) -> None:
    t0 = time.time()
    ...
    if not pid_raw:
        await q.answer(
            "Habit button expired. Please open Habits again.", show_alert=True
        )
        log.info("[PERF] TOTAL toggle time: %.0fms", (time.time() - t0) * 1000)
        return
    habit_page_id = _main()._restore_pid(pid_raw)
    message_id = q.message.message_id

    session = _main()._habit_selection_session(message_id)
...
async def _cb_h_done(q, parts, context) -> None:
    message_id = q.message.message_id
...
async def _do_log_habit(q, habit_page_id: str) -> None:
    ...
    if _main().already_logged_today(habit_page_id):
        try:
            ...
            log.warning("Habit dedupe UI update failed for %s: %s", habit_name, e)
    ...
    try:
        ...
        try:
            ...
            log.warning(
                "Habit log error UI update failed for %s: %s", habit_name, ui_error
            )
    ...
    asyncio.create_task(_main().check_and_notify_weekly_goals(q.bot, MY_CHAT_ID))


async def _cb_h_log(q, parts, context) -> None:
    pid_raw = q.data.removeprefix("h:log:").strip()
    if not pid_raw:
        await q.edit_message_text(
            "⚠️ Habit button expired. Please open 🎯 Habits again."
        )
        return
    await _do_log_habit(q, _main()._restore_pid(pid_raw))


async def _cb_h_legacy(q, parts, context) -> None:
    if len(parts) >= 3 and parts[1] == "check" and parts[2] == "cancel":
        await q.edit_message_text("✅ Habit check closed.")
        return
    if len(parts) != 3 or parts[1] != "log":
        return
    await _do_log_habit(q, _main()._restore_pid(parts[2]))
    ...
    try:
        await q.edit_message_reply_markup(
            reply_markup=kb.habit_buttons(
                ...
                prefix,
                page=int(page_str),
                selected=_main()._habit_selection_selected(q.message.message_id),
            )
        )
    except Exception as e:
...
async def _cb_el(q, parts, context) -> None:
    ...
    if not entry or entry.get("type") != "entertainment_log":
        await q.edit_message_text(
            "⚠️ This entertainment prompt expired — please send it again."
        )
        return
    payload = dict(entry.get("payload") or {})
    if action == "no":
...
async def _cb_d(q, parts, context) -> None:
...
async def _cb_tdc(q, parts, context) -> None:
    _, key = parts
    _main().todo_picker_map.pop(key, None)
...
async def _cb_td(q, parts, context) -> None:
    ...
    if key not in _main().todo_picker_map:
        await q.edit_message_text(
            "⚠️ This picker expired. Send `✅ To Do` again.", parse_mode="Markdown"
        )
        return
    tasks = _main().todo_picker_map[key]
    try:
        idx = int(idx_str)
        task = tasks[idx]
    except Exception:
...
async def _cb_dp(q, parts, context) -> None:
    _, key, idx_str = parts
    if key not in STATE.done_picker_map:
        await q.edit_message_text(
            "⚠️ This picker expired. Send `done` again.", parse_mode="Markdown"
        )
        return
    try:
...
async def _cb_dpp(q, parts, context) -> None:
    ...
    if key not in STATE.done_picker_map:
        ...
        return
    await q.edit_message_reply_markup(
        reply_markup=kb.done_picker_keyboard(
            key, STATE.done_picker_map, page=int(page_str)
        )
    )
    return


async def _cb_noop(q, parts, context) -> None:
    await q.answer()
    return


async def _cb_dpc(q, parts, context) -> None:
    STATE.done_picker_map.pop(parts[1], None)
    await q.edit_message_text("Done picker closed.")
    return


async def _cb_qp(q, parts, context) -> None:
    action = parts[1]
    ...
    if action == "done" and len(parts) == 3:
        ...
        tasks = _main()._get_today_tasks_for_palette()
    ...
    if action == "habits":
        ...
        await _main().send_daily_habits_list(q.bot)
        return

    if action == "notes":
        if NOTION_NOTES_DB:
            await q.edit_message_text(
                "📝 Notes connected. Choose an option:",
    ...
    if action == "weather":
        weather_text = _main().append_trip_reminders_to_text(
            fmt.format_weather_snapshot(), within_days=2
        )
...
async def _cb_qv(q, parts, context) -> None:
    try:
        ...
        if ordered and q.message:
            STATE.digest_map[q.message.message_id] = ordered
    except Exception as e:
...
async def _cb_digest_today(q, parts, context) -> None:
    try:
        tasks = notion_tasks.get_today_and_overdue_tasks(_notion(), NOTION_DB_ID)
...
_CALLBACK_PREFIX_HANDLERS: dict[str, CallbackHandler] = {
    ...
    "cancel_task": _cb_task_preview,
    "date_pick": _cb_date_pick,
    "tcancel": _cb_tcancel,
    "tw": _cb_tw,
    "twd": _cb_twd,
    "tms": _cb_tms,
    "tcl": _cb_tcl,
    "cf": _cb_cf,
    "kind_task": _cb_kind_task,
    "kind_refresh": _cb_kind_refresh,
    "mq": _cb_mq,
    "nq": _cb_nq,
    "kind_note": _cb_kind_note,
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    is_habit_multi = data.startswith("h:toggle:") or data in {
        "h:done",
        "h:check:cancel",
    }
    is_trip_multi = data.startswith("tw:")
    if not is_habit_multi and not is_trip_multi:
        try:
            await q.edit_message_reply_markup(reply_markup=None)
        except Exception:
            pass
    ...
    if parts and parts[0] == "hl":
        parts[0] = "hc"
    if len(parts) == 1 and data.startswith("cf_"):
        parts = ["cf", data.removeprefix("cf_")]
    if parts[:2] == ["cf", "A"]:
        parts = ["cf", "log_readiness", *parts[2:]]

    if data.startswith("h:toggle:"):
        await _cb_h_toggle(q, parts, context)
        return
    if data.startswith("h:log:"):
        await _cb_h_log(q, parts, context)
        return
